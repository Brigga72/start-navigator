...
const destinations = [
  ...
  {id:'sol', name:'SOL', deck:'4–5', area:'Deck 6 entrance · Absolute Zero', category:'shows', mustdo:true, icon:'⛸️', keywords:'sol ice show absolute zero skating', note:'SOL is performed in Absolute Zero. Passenger navigation uses the Absolute Zero entrance shown on Royal Caribbean Deck 6 plans.', confidence:'SHOW + VENUE VERIFIED; DECK 6 PASSENGER ENTRANCE VERIFIED', mapDeck:'6', mapNode:'absolute', route:[{kind:'walk',text:'Leave Cabin 7456 and follow the corridor toward the forward elevator bank shown on the Deck 7 map.'},{kind:'elevator',text:'Go down to Deck 5.'},{kind:'orient',text:'Orient toward the Absolute Zero entertainment area.'},{kind:'arrive',text:'Follow signs to Absolute Zero.'}]},
  ...
  {id:'absolute', name:'Absolute Zero', deck:'4–5', area:'Deck 6 entrance · Entertainment', category:'shows', icon:'🧊', keywords:'ice skating ice show laser tag sol', note:'Absolute Zero spans the lower entertainment decks. Passenger navigation uses the entrance explicitly marked on Royal Caribbean Deck 6 plans.', confidence:'VENUE + DECK 6 PASSENGER ENTRANCE VERIFIED', mapDeck:'6', mapNode:'absolute', route:[{kind:'walk',text:'Leave Cabin 7456 and follow the corridor toward the forward elevator bank shown on the Deck 7 map.'},{kind:'elevator',text:'Go down to Deck 5.'},{kind:'orient',text:'Orient toward the Absolute Zero arena.'},{kind:'arrive',text:'Follow signs to Absolute Zero.'}]},
  {id:'promenade', name:'Royal Promenade', deck:'5–6', area:'Midship · Shopping & Dining', category:'dining', icon:'🛍️', keywords:'royal promenade shops sorrentos pearl cafe', note:'The Royal Promenade is the ship’s central indoor public neighborhood on Decks 5–6. Navigation uses Pearl Café as a verified Deck 6 arrival anchor inside the Promenade.', confidence:'NEIGHBORHOOD VERIFIED; PEARL CAFE ARRIVAL ANCHOR VERIFIED', mapDeck:'6', mapNode:'promenade', route:[{kind:'walk',text:'Leave Cabin 7456 and follow the corridor toward the forward elevator bank shown on the Deck 7 map.'},{kind:'elevator',text:'Go down to Deck 6.'},{kind:'orient',text:'Orient toward the Royal Promenade.'},{kind:'arrive',text:'Enter the Promenade and use landmarks such as The Pearl to stay oriented.'}]},
  ...
  {id:'category6', name:'Category 6 Waterpark', deck:'16', area:'Thrill Island · Slides', category:'activities', icon:'🎢', keywords:'waterpark slides category 6 thrill island', note:'Category 6 slide access is on Deck 16 in Thrill Island. Navigation uses the staircase entrance to slides nearest the Deck 16 elevator approach, not the slide exits.', confidence:'DECK 16 SLIDE ACCESS VERIFIED FROM ROYAL CARIBBEAN DECK PLAN', mapDeck:'16', mapNode:'category6', route:[{kind:'walk',text:'Leave Cabin 7456 and follow the corridor toward the forward elevator bank shown on the Deck 7 map.'},{kind:'elevator',text:'Go up to Deck 16.'},{kind:'orient',text:'Orient aft toward Thrill Island / Category 6.'},{kind:'arrive',text:'Follow the specific slide signs; some access points continue to higher decks.'}]},
  {id:'royalbay', name:'Royal Bay Pool', deck:'15', area:'Midship · Chill Island', category:'pools', icon:'🏖️', keywords:'royal bay pool swim tonic chill island', note:'Royal Bay Pool is on Deck 15 in Chill Island, on the starboard side of the pool deck between the forward elevator area and Cantina Fresca.', confidence:'POOL LOCATION + DECK 15 APPROACH VERIFIED FROM ROYAL CARIBBEAN DECK PLAN', mapDeck:'15', mapNode:'royalbay', route:[{kind:'walk',text:'Leave Cabin 7456 and follow the corridor toward the forward elevator bank shown on the Deck 7 map.'},{kind:'elevator',text:'Go up to Deck 15.'},{kind:'orient',text:'Orient toward Chill Island / Royal Bay.'},{kind:'arrive',text:'Follow the pool deck signs to Royal Bay Pool.'}]},
  ...
  {id:'boleros', name:'Boleros', deck:'6', area:'Royal Promenade · Bar', category:'dining', icon:'💃', keywords:'boleros latin dance music', note:'Boleros is on Deck 6 on the port side of the Royal Promenade, opposite Schooner Bar and immediately forward of the elevator bank.', confidence:'VENUE + DECK 6 ENTRANCE VERIFIED FROM ROYAL CARIBBEAN DECK PLAN', mapDeck:'6', mapNode:'boleros', route:[]},
  ...
  {id:'aotheater', name:'Adventure Ocean', deck:'6', area:'Deck 6 · Youth Program', category:'shows', icon:'🎭', keywords:'adventure ocean kids youth program family', note:'Adventure Ocean is on Deck 6. The official deck plan marks its entrance immediately aft of the elevator bank on the port side.', confidence:'STAR OF THE SEAS ADVENTURE OCEAN + DECK 6 ENTRANCE VERIFIED', mapDeck:'6', mapNode:'adventureocean', route:[]},
  {id:'sportscourt', name:'Sports Court', deck:'17', area:'Aft · Sports', category:'activities', icon:'🏀', keywords:'sports court basketball pickleball competition', note:'Sports Court is on Deck 17 aft. Navigation uses the aft Deck 17 elevator area and the Cloud 17 outer walkway shown on the Royal Caribbean deck plan.', confidence:'DECK 17 VENUE + APPROACH VERIFIED FROM ROYAL CARIBBEAN DECK PLAN', mapDeck:'17', mapNode:'sportscourt', route:[]}
];
...
const PROD_ROUTE_ALIASES_V0285={
  ...
  chops:'Chops Grille',
  pearl:'The Pearl',
  royalpromenade:'Pearl Cafe',
  promenade:'Pearl Cafe',
  ...
  fetamed:'Aqua Theater',
  cremedelacrepe:'Aqua Theater',
...
function prodResolutionDiagnosticV02813(loc,node){
  if(!loc)return {location:null,node:null,deckMatch:null,reason:'missing_location'};
  const alias=PROD_ROUTE_ALIASES_V0285[loc.id]||PROD_DEST_ALIASES_V026[loc.id]||null;
  const locationDeck=String(loc.mapDeck||loc.deck||'');
  const graphDeck=node?String(node.deck||''):'';
  const deckMatch=!!node&&locationDeck===graphDeck;
  return {
    location:{id:loc.id,name:loc.name,deck:locationDeck,area:loc.area||'',alias:alias||null},
    node:node?{id:node.id,label:node.label,deck:graphDeck,panel:node.panel,type:node.type}:null,
    deckMatch,
...
function prodRouteV0285(fromId,d){
  ...
  debugPushV0272('production_endpoint_resolution',{
    role:'start',
    ...prodResolutionDiagnosticV02813(from,start)
  });
  debugPushV0272('production_endpoint_resolution',{
    role:'destination',
    ...prodResolutionDiagnosticV02813(d,end)
  });

  const startResolution=prodResolutionDiagnosticV02813(from,start);
  if(start&&startResolution.deckMatch===false){
    debugPushV0272('production_start_mismatch_blocked',{
      location:startResolution.location,
      graphEntrance:startResolution.node,
      reason:'current_location_deck_does_not_match_verified_graph_entrance',
      action:'production_weighted_route_blocked_use_conservative_fallback'
    });
    return null;
  }
...
function guidedMapFor(d,idx){
  ...
  if(confidence.level==='verified'){
    if(step.kind==='elevator'||step.kind==='stairs'){
      ...
      return `<div class="deck-change-card compact-deck-change"><div class="deck-change-main"><div class="deck-change-icon">${isStairs?'🪜':'🛗'}</div><div class="deck-from">DECK ${esc(fromDeck)}</div><div class="deck-arrow">${direction}</div><div class="deck-to">DECK ${esc(toDeck)}</div></div><div class="deck-change-ship" aria-hidden="true"><div class="mini-deck-label">Deck ${esc(fromDeck)}</div><div class="mini-ship-hull"><span class="mini-aft">AFT</span><span class="mini-elevator">${isStairs?'🪜':'🛗'}</span><span class="mini-fwd">FWD</span><i class="mini-deck-arrow">${direction}</i><b>Deck ${esc(toDeck)}</b></div></div></div>`;
    }
...
function openCurrentVerifiedMapV0295(){
  const d=destinations.find(x=>x.id===guidedState.destId);if(!d)return;
  const idx=Math.max(0,Math.min(guidedState.step,d.route.length-1));
  const step=d.route[idx];if(!step)return;
  const full=prodMapPanelV026(step,true);if(!full)return;
  el('overlayTitle').textContent='Full Deck Map';
  el('overlayMap').innerHTML=`<div class="route-full-map-v0295">${full}</div>`;
  el('mapOverlay').classList.add('show');
  el('mapOverlay').setAttribute('aria-hidden','false');
}
...
function renderGuidedRoute(){
  ...
  const canExpandVerifiedMap=s.kind==='walk'&&stepConfidenceV027(s).level==='verified'&&!!s.v026;
  const verifiedMapAction=canExpandVerifiedMap?'<div class="verified-map-actions-v0295"><button id="viewFullVerifiedMapV0295" type="button">↗ View Full Map</button></div>':'';
  ...
  if(el('viewFullVerifiedMapV0295'))el('viewFullVerifiedMapV0295').onclick=openCurrentVerifiedMapV0295;
...
const ROUTE_PROFILES_V0284={
  ...
  accessible:{label:'No Stairs',desc:'Excludes staircase edges; elevators are used for deck changes when available.',stairsPenalty:Infinity,elevatorPenalty:180,stairsBlocked:true}
};
...
function findWeightedPathV0284(start,end,state=loadShipnetV020(),profileKey=shipnetRouteProfileV0284){
  ...
  const distance=ids.slice(1).reduce((sum,id)=>{const a=by[prev[id]],b=by[id];if(!a||!b)return sum;if(a.panel!==b.panel)return sum;return sum+Math.hypot((a.x||0)-(b.x||0),(a.y||0)-(b.y||0))},0);
...
function bindShipnetV020(){
  ...
  document.querySelectorAll('[data-shipmode]').forEach(b=>b.onclick=()=>{shipnetModeV020=b.dataset.shipmode;if(shipnetModeV020!=='path')shipnetPathLastV020=null;if(shipnetModeV020==='connect'){if(shipnetSelectedV020&&!shipnetConnectFirstV020)shipnetConnectFirstV020=shipnetSelectedV020}else shipnetConnectFirstV020=null;if(shipnetModeV020!=='stairslink')shipnetStairFirstV024=null;renderShipNetworkV020()});
...
document.addEventListener('click',e=>{
  ...
  const view=e.target.closest('[data-view]');if(view){if(view.dataset.view==='home'){returnHomeCleanV0297();}else{navigate(view.dataset.view);}return}
...
document.addEventListener('change',e=>{if(e.target&&e.target.id==='drinkCategorySelect'){drinkFilter=e.target.value;localStorage.setItem(DRINK_FILTER_KEY,drinkFilter);renderDrinks();}},true);
...
const DRINK_FILTER_KEY='cruise-nav-drink-filter-v02915';
const DRINK_PROFILE_KEY='cruise-nav-drink-profile-v015';
const DRINK_PROFILES={daniel:'Daniel',wife:'Wife',both:'Both'};
const DRINKS=[
 {id:'marshmallow-old-fashioned',name:'Toasted Marshmallow Old Fashioned',emoji:'🥃',style:'Whiskey',venue:'Schooner Bar',routeId:'schooner',sweet:'Rich & sweet',why:'A dessert-like Old Fashioned direction and a strong Schooner pick.',order:'Ask for the Toasted Marshmallow Old Fashioned, or a sweet bourbon Old Fashioned if it is not listed.',community:'CRUISER FAVORITE',verifiedVenue:true},
 {id:'new-fashioned',name:'New Fashioned',emoji:'🥃',style:'Whiskey',venue:'Schooner Bar',routeId:'schooner',sweet:'Grapefruit & raspberry',why:'Royal Caribbean highlights this fruitier spin on an Old Fashioned at Schooner Bar.',order:'Ask for the New Fashioned, a fruit-forward Old Fashioned with grapefruit and raspberry notes.',community:'ROYAL MENU',verifiedVenue:true},
 {id:'tropical-old-fashioned',name:'Tropical Old Fashioned',emoji:'🍍',style:'Whiskey',venue:'Schooner Bar',routeId:'schooner',sweet:'Tropical & spirit-forward',why:'Royal Caribbean describes a tropical Old Fashioned variation with cherry, pineapple, bitters and coconut rum.',order:'Ask for Schooner’s tropical Old Fashioned variation.',community:'ROYAL MENU',verifiedVenue:true},
 {id:'zacapa-daiquiri',name:'Zacapa Rum Daiquiri',emoji:'🍹',style:'Rum',venue:'Schooner Bar',routeId:'schooner',sweet:'Crisp & rum-forward',why:'A Royal Caribbean Schooner menu highlight for someone who wants a serious daiquiri rather than a frozen one.',order:'Ask for the Zacapa rum daiquiri, shaken and served up.',community:'ROYAL MENU',verifiedVenue:true},
 {id:'papa-doble',name:'Papa Doble',emoji:'🍸',style:'Rum',venue:'Schooner Bar',routeId:'schooner',sweet:'Lychee & grapefruit',why:'Royal Caribbean highlights this Bacardi-based Schooner cocktail with lychee and grapefruit.',order:'Ask for the Papa Doble.',community:'ROYAL MENU',verifiedVenue:true},
 {id:'lavender-daiquiri',name:'Lavender Daiquiri',emoji:'💜',style:'Rum',venue:'Schooner Bar',routeId:'schooner',sweet:'Floral & bright',why:'A recurring Schooner favorite in cruiser discussions.',order:'Ask whether the bartender can make the lavender daiquiri.',community:'CRUISER FAVORITE',verifiedVenue:true},
 {id:'desert-pear-margarita',name:'Desert Pear Margarita',emoji:'🌵',style:'Tequila',venue:'The Lime & Coconut or full-service bar',routeId:'',sweet:'Fruity & tart',why:'Royal Caribbean lists a Desert Pear Rita among Lime & Coconut favorites.',order:'Ask for the Desert Pear Rita, fruity, tart and tequila-based.',community:'ROYAL MENU'},
 {id:'watermelon-rita',name:'Watermelon Rita',emoji:'🍉',style:'Tequila',venue:'The Lime & Coconut',routeId:'',sweet:'Watermelon & citrus',why:'An official Lime & Coconut tequila option built around watermelon.',order:'Ask for the Watermelon Rita.',community:'ROYAL MENU'},
 {id:'smokey-rita',name:'Smokey Rita',emoji:'🔥',style:'Tequila',venue:'Pool or full-service bar',routeId:'',sweet:'Smoky & tart',why:'Royal Caribbean highlights this mezcal margarita direction on its pool-bar menus.',order:'Ask for a mezcal Smokey Rita, or a smoky mezcal margarita.',community:'ROYAL MENU IDEA'},
 {id:'pepperita',name:'Strawberry Mezcal Pepperita',emoji:'🌶️',style:'Tequila',venue:'Pool or full-service bar',routeId:'',sweet:'Strawberry, smoky & spicy',why:'A Royal Caribbean pool-bar menu highlight for adventurous margarita drinkers.',order:'Ask for a strawberry mezcal margarita with a little pepper or spice.',community:'ROYAL MENU IDEA'},
 {id:'lime-coconut',name:'Lime & Coconut',emoji:'🥥',style:'Tropical',venue:'The Lime & Coconut',routeId:'',sweet:'Creamy & tropical',why:'The signature namesake drink at Royal Caribbean’s Lime & Coconut.',order:'Ask for the signature Lime & Coconut.',community:'ROYAL SIGNATURE'},
 {id:'blue-wave',name:'Blue Wave',emoji:'🌊',style:'Vodka',venue:'The Lime & Coconut',routeId:'',sweet:'Pineapple & citrus',why:'Royal Caribbean describes this as a simple poolside combination of vodka, blue curaçao and pineapple.',order:'Ask for a Blue Wave.',community:'ROYAL MENU'},
 {id:'ultimate-mai-tai',name:'Ultimate Mai Tai',emoji:'🌺',style:'Rum',venue:'Pool or full-service bar',routeId:'',sweet:'Tropical & rum-forward',why:'A Royal Caribbean pool-bar menu highlight and an easy vacation drink.',order:'Ask for the Ultimate Mai Tai, or the bar’s best Mai Tai.',community:'ROYAL MENU IDEA'},
 {id:'rum-runner',name:'Rum Runner',emoji:'🏝️',style:'Rum',venue:'The Lime & Coconut or pool bar',routeId:'',sweet:'Fruity & boozy',why:'Royal Caribbean lists Rum Runners among its Lime & Coconut rum favorites.',order:'Ask for a Rum Runner.',community:'ROYAL MENU'},
 {id:'blue-hawaiian',name:'Blue Hawaiian',emoji:'💙',style:'Rum',venue:'The Lime & Coconut or pool bar',routeId:'',sweet:'Sweet & tropical',why:'A familiar Royal Caribbean poolside rum option.',order:'Ask for a Blue Hawaiian.',community:'ROYAL MENU'},
 {id:'coconut-mango-daiquiri',name:'Coconut Mango Daiquiri',emoji:'🥭',style:'Frozen',venue:'Pool or blender bar',routeId:'',sweet:'Mango & coconut',why:'Royal Caribbean specifically highlights this fruity daiquiri on pool-bar menus.',order:'Ask for a coconut mango daiquiri, frozen if available.',community:'ROYAL MENU IDEA'},
 {id:'miami-vice',name:'Miami Vice',emoji:'🍓',style:'Frozen',venue:'Ask at a blender bar',routeId:'',sweet:'Strawberry & coconut',why:'Half strawberry daiquiri and half piña colada, a long-running cruise favorite.',order:'Ask for a Miami Vice, half strawberry daiquiri and half piña colada.',community:'CLASSIC CRUISE PICK'},
 {id:'frozen-mojito',name:'Frozen Mojito',emoji:'🌿',style:'Frozen',venue:'Ask at a blender bar',routeId:'',sweet:'Minty & refreshing',why:'A refreshing cruiser favorite when you want frozen without going creamy.',order:'Ask for a frozen mojito.',community:'COMMUNITY PICK'},
 {id:'classic-mojito',name:'Classic Mojito',emoji:'🌿',style:'Rum',venue:'Full-service bar',routeId:'',sweet:'Mint, lime & crisp',why:'Royal Caribbean calls the mojito a go-to at its Latin bars, and most full bars can make the classic.',order:'Ask for a classic mojito, light on syrup if you want it less sweet.',community:'ROYAL CLASSIC'},
 {id:'dark-stormy',name:"Dark 'n' Stormy",emoji:'⛈️',style:'Rum',venue:'Full-service bar',routeId:'',sweet:'Ginger & lime',why:'Royal Caribbean highlights this ginger-beer rum classic on multiple bar menus.',order:"Ask for a Dark 'n' Stormy with dark rum, lime and ginger beer.",community:'ROYAL CLASSIC'},
 {id:'caribbean-mule',name:'Caribbean Mule',emoji:'🫚',style:'Tropical',venue:'Ask at a full-service bar',routeId:'',sweet:'Ginger & citrus',why:'A cruise-friendly mule direction for something refreshing without going frozen.',order:'Ask for a Caribbean-style mule with rum, ginger beer and lime.',community:'COMMUNITY PICK'},
 {id:'espresso-martini',name:'Espresso Martini',emoji:'☕',style:'Coffee',venue:'Rye & Bean',routeId:'ryebean',sweet:'Coffee & cocktail',why:'Star specifically promotes Rye & Bean as a coffee-and-cocktail venue.',order:'Ask for an espresso martini, and ask the Rye & Bean bartender about their house variation.',community:'STAR PICK',verifiedVenue:true},
 {id:'flat-white',name:'Flat White',emoji:'☕',style:'Coffee',venue:'Rye & Bean',routeId:'ryebean',sweet:'Smooth & coffee-forward',why:'Royal Caribbean specifically calls out the flat white at Rye & Bean.',order:'Ask for a flat white.',community:'STAR VERIFIED',verifiedVenue:true},
 {id:'cappuccino',name:'Cappuccino',emoji:'☕',style:'Coffee',venue:'Rye & Bean',routeId:'ryebean',sweet:'Foamy & balanced',why:'A classic specialty coffee and a natural morning order at Rye & Bean.',order:'Ask for a cappuccino, and add your preferred syrup only if you want it sweeter.',community:'COFFEE CLASSIC',verifiedVenue:true},
 {id:'latte',name:'Caffè Latte',emoji:'🥛',style:'Coffee',venue:'Rye & Bean',routeId:'ryebean',sweet:'Creamy & mellow',why:'A familiar specialty coffee choice for a softer espresso drink.',order:'Ask for a latte, hot or iced.',community:'COFFEE CLASSIC',verifiedVenue:true},
 {id:'iced-coffee',name:'Iced Coffee',emoji:'🧊',style:'Coffee',venue:'Rye & Bean',routeId:'ryebean',sweet:'Cold & refreshing',why:'A straightforward daytime caffeine option when you do not want a cocktail.',order:'Ask for iced coffee and customize milk or sweetener to taste.',community:'COFFEE CLASSIC',verifiedVenue:true},
 {id:'americano',name:'Americano',emoji:'☕',style:'Coffee',venue:'Rye & Bean',routeId:'ryebean',sweet:'Bold & unsweetened',why:'A simple espresso-and-water option for a stronger coffee taste without lots of milk.',order:'Ask for an Americano, hot or iced.',community:'COFFEE CLASSIC',verifiedVenue:true},
 {id:'macchiato',name:'Macchiato',emoji:'☕',style:'Coffee',venue:'Rye & Bean',routeId:'ryebean',sweet:'Espresso-forward',why:'A compact specialty coffee for someone who wants more espresso character.',order:'Ask for a macchiato.',community:'COFFEE CLASSIC',verifiedVenue:true},
 {id:'salted-caramel-espresso-martini',name:'Salted Caramel Espresso Martini',emoji:'🍮',style:'Coffee',venue:'Ask at Rye & Bean',routeId:'ryebean',sweet:'Coffee, caramel & sweet',why:'Royal Caribbean has published a salted caramel espresso martini on its fleet menus, making it a strong Rye & Bean request.',order:'Ask whether they can make a salted caramel espresso martini.',community:'ROYAL MENU IDEA',verifiedVenue:true},
 {id:'baileys-espresso-martini',name:'Baileys Espresso Martini',emoji:'🤎',style:'Coffee',venue:'Ask at Rye & Bean',routeId:'ryebean',sweet:'Creamy coffee cocktail',why:'A simple bartender variation using the coffee-cocktail focus at Rye & Bean.',order:'Ask for an espresso martini with Baileys.',community:'BARTENDER REQUEST',verifiedVenue:true},
 {id:'chocolate-espresso-martini',name:'Chocolate Espresso Martini',emoji:'🍫',style:'Coffee',venue:'Ask at Rye & Bean',routeId:'ryebean',sweet:'Chocolate & coffee',why:'A dessert-style variation that fits Rye & Bean’s coffee-infused cocktail concept.',order:'Ask for a chocolate espresso martini, or an espresso martini with chocolate notes.',community:'BARTENDER REQUEST',verifiedVenue:true},
 {id:'coconut-espresso-martini',name:'Coconut Espresso Martini',emoji:'🥥',style:'Coffee',venue:'Ask at Rye & Bean',routeId:'ryebean',sweet:'Coconut & coffee',why:'A tropical coffee-cocktail variation suited to a cruise bar.',order:'Ask for an espresso martini with coconut if the bartender has the ingredients.',community:'BARTENDER REQUEST',verifiedVenue:true},
 {id:'irish-coffee',name:'Irish Coffee',emoji:'🇮🇪',style:'Coffee',venue:'Ask at Rye & Bean',routeId:'ryebean',sweet:'Warm, whiskey & coffee',why:'A classic hot coffee cocktail and an easy bartender request.',order:'Ask for an Irish coffee with Irish whiskey.',community:'COCKTAIL CLASSIC',verifiedVenue:true},
 {id:'carajillo',name:'Carajillo',emoji:'🟠',style:'Coffee',venue:'Ask at Rye & Bean',routeId:'ryebean',sweet:'Espresso & liqueur',why:'A compact coffee cocktail worth asking for when you want something less creamy than an espresso martini.',order:'Ask whether they can make a carajillo with espresso and Licor 43.',community:'BARTENDER REQUEST',verifiedVenue:true},
 {id:'rye-bean-choice',name:"Rye & Bean Bartender's Choice",emoji:'🎲',style:'Coffee',venue:'Rye & Bean',routeId:'ryebean',sweet:'Your choice',why:'Royal Caribbean describes Rye & Bean as having an expansive range of coffee-infused cocktails.',order:'Tell the bartender your preferred spirit and sweetness level, then ask for their favorite coffee cocktail.',community:'STAR VENUE IDEA',verifiedVenue:true},
 {id:'old-fashioned',name:'Classic Old Fashioned',emoji:'🥃',style:'Whiskey',venue:'Schooner Bar or full-service bar',routeId:'schooner',sweet:'Spirit-forward',why:'A reliable benchmark drink, especially at Schooner where Royal Caribbean emphasizes Old Fashioned variations.',order:'Ask for a classic bourbon Old Fashioned.',community:'CLASSIC',verifiedVenue:true},
 {id:'manhattan',name:'Manhattan',emoji:'🥃',style:'Whiskey',venue:'Full-service bar',routeId:'',sweet:'Strong & smooth',why:'Royal Caribbean highlights Manhattans on several bar menus.',order:'Ask for a bourbon Manhattan, up.',community:'ROYAL CLASSIC'},
 {id:'buffalo-manhattan',name:'Buffalo Manhattan',emoji:'🦬',style:'Whiskey',venue:'Pub or full-service bar',routeId:'',sweet:'Bourbon & bitters',why:'Royal Caribbean’s English Pub menu highlights a Buffalo Trace Manhattan variation.',order:'Ask whether they can make the Buffalo Manhattan, or use Buffalo Trace in a classic Manhattan.',community:'ROYAL MENU IDEA'},
 {id:'kentucky-burgundy',name:'Kentucky Burgundy',emoji:'🍓',style:'Whiskey',venue:'Full-service bar',routeId:'',sweet:'Berry, lime & bourbon',why:'Royal Caribbean describes this bourbon cocktail as balancing spirit, berry sweetness and citrus.',order:'Ask for Kentucky Burgundy if available, or a bourbon cocktail with cassis, lime and strawberry.',community:'ROYAL MENU IDEA'},
 {id:'aperol-spritz',name:'Aperol Spritz',emoji:'🧡',style:'Sparkling',venue:'Full-service bar',routeId:'',sweet:'Bitter, bubbly & light',why:'A Royal Caribbean bar staple and an easy lower-intensity afternoon choice.',order:'Ask for an Aperol Spritz.',community:'ROYAL CLASSIC'},
 {id:'french-75',name:'French 75',emoji:'🥂',style:'Sparkling',venue:'Bubbles or full-service bar',routeId:'',sweet:'Crisp & citrusy',why:'A natural sparkling cocktail choice when you want something lighter and celebratory.',order:'Ask for a French 75, and try a touch of lavender syrup if the bartender has it.',community:'BARTENDER REQUEST'},
 {id:'gin-tonic',name:'Gin & Tonic',emoji:'🍋',style:'Gin',venue:'Full-service bar',routeId:'',sweet:'Dry & refreshing',why:'Simple, dependable and specifically called out across Royal Caribbean classic-cocktail venues.',order:'Ask the bartender which gin they recommend for a crisp gin and tonic.',community:'CLASSIC'},
 {id:'blood-orange-negroni',name:'Blood Orange Negroni',emoji:'🍊',style:'Gin',venue:'Full-service bar',routeId:'',sweet:'Bitter & citrusy',why:'Royal Caribbean highlights this citrus twist on a Negroni on several bar menus.',order:'Ask for the blood orange Negroni if available, or a citrus-forward Negroni.',community:'ROYAL MENU IDEA'},
 {id:'royal-singapore-sling',name:'Royal Singapore Sling',emoji:'🍒',style:'Gin',venue:'Full-service bar',routeId:'',sweet:'Fruity & complex',why:'Royal Caribbean describes its Singapore Sling variation as a guest favorite.',order:'Ask whether they can make the Royal Singapore Sling.',community:'ROYAL MENU IDEA'},
 {id:'james-bond-martini',name:'James Bond Martini',emoji:'🕶️',style:'Martini',venue:'Pub or full-service bar',routeId:'',sweet:'Dry & strong',why:'Royal Caribbean’s pub menu highlights this vodka-and-gin martini style.',order:'Ask for the James Bond Martini if listed, or a Vesper-style martini.',community:'ROYAL MENU IDEA'},
 {id:'lavender-martini',name:'Lavender Martini',emoji:'🍸',style:'Martini',venue:'Ask at Schooner Bar',routeId:'schooner',sweet:'Floral',why:'A popular cruiser request for fans of floral cocktails.',order:'Ask whether the bartender can make a lavender martini.',community:'COMMUNITY PICK'},
 {id:'goombay-smash',name:'Goombay Smash',emoji:'🍍',style:'Tropical',venue:'Ask at a full-service bar',routeId:'',sweet:'Pineapple & rum',why:'A popular cruise recommendation for something unmistakably tropical.',order:'Ask for a Goombay Smash, rum-forward with pineapple.',community:'COMMUNITY PICK'},
 {id:'pina-colada',name:'Piña Colada',emoji:'🥥',style:'Frozen',venue:'Pool or blender bar',routeId:'',sweet:'Creamy & coconut',why:'The classic frozen cruise drink and a useful baseline for deciding how sweet you like frozen cocktails.',order:'Ask for a piña colada, or add a dark-rum floater if you want it stronger.',community:'CRUISE CLASSIC'},
 {id:'strawberry-daiquiri',name:'Strawberry Daiquiri',emoji:'🍓',style:'Frozen',venue:'Pool or blender bar',routeId:'',sweet:'Berry & frozen',why:'A straightforward frozen favorite that is also easy to order alcohol-free.',order:'Ask for a frozen strawberry daiquiri.',community:'CRUISE CLASSIC'},
 {id:'moscow-mule',name:'Moscow Mule',emoji:'🫚',style:'Vodka',venue:'Full-service bar',routeId:'',sweet:'Ginger & lime',why:'Royal Caribbean highlights the Moscow Mule as a refreshing vodka classic.',order:'Ask for a Moscow Mule with plenty of lime.',community:'ROYAL CLASSIC'},
 {id:'passion-city',name:'Passion in the City',emoji:'❤️',style:'Vodka',venue:'Full-service bar',routeId:'',sweet:'Passion fruit & raspberry',why:'Royal Caribbean highlights this fruity vodka combination on one of its specialty bar menus.',order:'Ask if they can make Passion in the City, or a vodka cocktail with passion fruit and raspberry.',community:'ROYAL MENU IDEA'},
 {id:'blueberry-nojito',name:'Blueberry Nojito',emoji:'🫐',style:'No Alcohol',venue:'Ask the bartender',routeId:'',sweet:'Berry & mint',why:'A refreshing zero-proof direction for mojito fans.',order:'Ask for a blueberry nojito, no alcohol.',community:'ZERO-PROOF PICK'},
 {id:'virgin-miami-vice',name:'Virgin Miami Vice',emoji:'🍓',style:'No Alcohol',venue:'Blender bar',routeId:'',sweet:'Strawberry & coconut',why:'The cruise classic works well without alcohol and still feels like a vacation drink.',order:'Ask for a virgin Miami Vice.',community:'ZERO-PROOF PICK'},
 {id:'virgin-mojito',name:'Virgin Mojito',emoji:'🌿',style:'No Alcohol',venue:'Full-service bar',routeId:'',sweet:'Mint & lime',why:'A simple, refreshing zero-proof order available at most capable bars.',order:'Ask for a virgin mojito, light on syrup if preferred.',community:'ZERO-PROOF PICK'},
 {id:'bartenders-choice',name:"Bartender's Choice",emoji:'🎲',style:'Surprise',venue:'Schooner Bar or 1400 Lobby Bar',routeId:'schooner',sweet:'You choose the direction',why:'Royal Caribbean explicitly recommends telling bartenders your favorite spirit and letting them surprise you.',order:'Tell the bartender your spirit, sweetness level and one flavor you like, then ask them to surprise you.',community:'ROYAL PRO TIP',verifiedVenue:true}
]
...
const exploreVenues = [
  ...
  {id:'pigoutbbq', name:'Pig Out BBQ', icon:'🐷', deck:'15', deckLabel:'15', area:'Inside AquaDome Market', kind:'Dining', included:true, type:'AquaDome Market food stand', food:'Smoky pulled pork, beef brisket and BBQ favorites.', best:'Lunch · Casual · BBQ', detail:'One of five included food stands inside AquaDome Market on Star of the Seas.', related:'AquaDome Market', keywords:'pig out bbq barbecue pulled pork brisket aquadome market included', routeArea:'Forward · AquaDome'},
  {id:'maithai', name:'Mai Thai', icon:'🍜', deck:'15', deckLabel:'15', area:'Inside AquaDome Market', kind:'Dining', included:true, type:'AquaDome Market food stand', food:'Thai favorites including pad thai and beef panang.', best:'Lunch · Casual · Thai', detail:'One of five included food stands inside AquaDome Market on Star of the Seas.', related:'AquaDome Market', keywords:'mai thai pad thai beef panang asian aquadome market included', routeArea:'Forward · AquaDome'},
  {id:'lacocinita', name:'La Cocinita', icon:'🫓', deck:'15', deckLabel:'15', area:'Inside AquaDome Market', kind:'Dining', included:true, type:'AquaDome Market food stand', food:'Latin American favorites including empanadas and arepas.', best:'Lunch · Casual · Latin American', detail:'One of five included food stands inside AquaDome Market on Star of the Seas.', related:'AquaDome Market', keywords:'la cocinita empanadas arepas latin south american aquadome market included', routeArea:'Forward · AquaDome'},
  {id:'fetamed', name:'Feta Mediterranean', icon:'🥙', deck:'15', deckLabel:'15', area:'Inside AquaDome Market', kind:'Dining', included:true, type:'AquaDome Market food stand', food:'Fresh pitas and Mediterranean bowls.', best:'Lunch · Casual · Mediterranean', detail:'One of five included food stands inside AquaDome Market on Star of the Seas.', related:'AquaDome Market', keywords:'feta mediterranean pita pitas bowls aquadome market included', routeArea:'Forward · AquaDome'},
  {id:'cremedelacrepe', name:'Crème de la Crepe', icon:'🥞', deck:'15', deckLabel:'15', area:'Inside AquaDome Market', kind:'Dining', included:true, type:'AquaDome Market food stand', food:'Sweet and savory made-to-order French crêpes.', best:'Breakfast · Dessert · Crêpes', detail:'One of five included food stands inside AquaDome Market on Star of the Seas.', related:'AquaDome Market', keywords:'creme crème de la crepe crêpe crepes french sweet savory aquadome market included', routeArea:'Forward · AquaDome'},
...
function mapNodeForExplore(id){
  return ({aquadome:'aquadome',overlook:'aquadome',ryebean:'aquadome',aquadomemarket:'aquadome',pigoutbbq:'aquadome',maithai:'aquadome',lacocinita:'aquadome',fetamed:'aquadome',cremedelacrepe:'aquadome',hooked:'aquadome',torque:'aquadome',pirates:'aquadome',windjammer:'windjammer',royalbay:'royalbay',hideaway:'royalbay',elloco:'royalbay',basecamp:'basecamp',basecampbar:'basecamp',centralpark:'centralpark',chops:'centralpark',izumi:'centralpark',lincoln:'centralpark',parkcafe:'centralpark',royalpromenade:'promenade',sorrentos:'promenade',pearlcafe:'promenade',giovannis:'promenade',playmakers:'promenade',attic:'promenade',starbucks:'promenade',guestservices:'guestservices',royaltheater:'theater',backfuture:'theater',absolutezero:'absolute',sol:'absolute',surfsideeatery:'surfside',surfbites:'surfside',pier7:'surfside'})[id]||null;
}
...
function venueCardV07(v){
  const cost=v.included?'✓ Included in cruise fare':'$ Additional purchase';
...
function openVenueV07(id){
  ...
  el('overlayMap').innerHTML=`<div class="venue-detail-v07"><div class="venue-detail-title"><div class="venue-detail-icon">${v.icon}</div><div><div class="eyebrow">${v.kind.toUpperCase()} · DECK ${esc(v.deckLabel)}</div><h2>${esc(v.name)}</h2><div class="venue-detail-area">${esc(v.area)}</div></div></div><div class="detail-chips"><span>${v.included?'✓ Included in cruise fare':'$ Additional purchase'}</span><span>📍 Deck ${esc(v.deckLabel)}</span><span>🧭 ${esc(v.best)}</span></div><div class="detail-block"><h3>What is it?</h3><p>${esc(v.detail)}</p></div><div class="detail-block"><h3>Food / experience</h3><p>${esc(v.food)}</p></div><div class="detail-block"><h3>Why remember it</h3><p>${esc(v.related)}</p></div><div class="venue-actions-v07"><button class="primary-venue-btn" data-venue-route="${v.id}">🧭 Take Me There</button><button class="secondary-venue-btn" data-add-mustdo="${v.id}">⭐ Add to Must-Do</button></div><div class="nearby-v07"><h3>Also nearby</h3>${related.length?related.map(r=>`<button class="nearby-item-v07" data-venue-v07="${r.id}">${r.icon} ${r.name}<span>›</span></button>`).join(''):'<p>No mapped nearby suggestions yet.</p>'}</div><div class="explore-source-v07">For current hours, reservations, showtimes and sailing-specific availability, check the Royal App.</div></div>`;
  el('mapOverlay').classList.add('show');el('mapOverlay').setAttribute('aria-hidden','false');
}
